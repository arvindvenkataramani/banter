...
export default definePluginEntry({
  ...
  register(api) {
    const config = (api as any).config?.plugins?.entries?.["voice-mode"]?.config ?? {};
    const prefix: string = config.sessionKeyPrefix ?? "agent:voice:";
    ...
    api.on(
      "before_prompt_build",
      (_event: any, ctx: any) => {
        if (!ctx.sessionKey?.startsWith(prefix)) return {};
